...
void print(){
  ...
  printf("3)Display a rule\n");
  printf("4)Modifify rule rating\n");
  printf("5)Discussion\n");
  printf("9)Quit\n");
}
int searchInRead(int size, char *p,Read *info){
  int k=-1;
  int i;
  for(i=0;i<=size;i++){
    if(strcmp(p,(info+i)->keywords)==0)
      k=i;
...
int main(int argc, char *argv[]){
  ...
  char key[10], sentence[100], feedBack[2];
  float frate;
  int i=0,z=0;
  char command[2];
